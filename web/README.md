...
## Dois objetos de dado (set/2026)

O site é quente e fotográfico de propósito, mas onde ele faz uma afirmação
numérica o número tem que aparecer:

- **A noite da geada** (`.noite`, na faixa escura): os dois números do título
  — "Geada às 5. Aviso às 22." — são dois pontos numa curva real da noite,
  desenhada em creme sobre a via láctea. A linha tracejada em cereja é o
  limiar de 3 °C; a faixa sombreada é o trecho abaixo dele; o ponto cereja é
  a mínima. Dois SVGs (desktop 1280×400, celular 400×300) porque texto em SVG
  não escala bem. O título dizia "às 4" enquanto o chip da seção anterior
  dizia "às 05h": numa página que vive de mostrar a fonte, os dois números
  passam a bater.
- **O cartaz do balcão** (`.cartaz`, na seção de cooperativas): a lista diz
  "um número, um QR no balcão" — agora a folha A4 aparece embaixo da foto do
  armazém, com o QR de verdade servido por `/api/qr` (o mesmo número público
  do resto do site). Folha clara, sombra curta de papel, 190px: é um objeto
  sobre o balcão, não um banner.
