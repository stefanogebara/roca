...
describe('formatDigest', () => {
  ...
  it('renders the weekly cohort block when present (WAU trend, D7, habits)', () => {
    const t = formatDigest({
      ...
      cohort: {
        ...
        d7Vouched: { size: 3, retained: 2, rate: 2 / 3 },
        d7Organic: { size: 4, retained: 1, rate: 1 / 4 },
        newSources: [
          { source: 'tec-jose', count: 3 },
          { source: 'orgânico', count: 4 },
        ],
        habits: [
    ...
    expect(t).toMatch(/retenção D7: 3\/7 \(43%\) — vouchados 2\/3 · orgânicos 1\/4/);
    expect(t).toMatch(/🧭 Origem \(novos 7d\): tec-jose 3 · orgânico 4/);
  ...
  it('an empty D7 cohort reads as "sem coorte ainda", never 0%', () => {
    const empty = { size: 0, retained: 0, rate: null };
    const t = formatDigest({
      ...base,
      cohort: {
        wau: 1,
    ...
    expect(t).not.toMatch(/🧭/); // no sources line when nothing arrived
  });
