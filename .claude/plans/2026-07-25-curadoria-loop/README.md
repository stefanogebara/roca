...
- [04/ago] **Pesquisa voz (pedido do founder): agente de ligação para leads B2B — ElevenLabs Agents é o caminho de menor atrito (zero infra nossa), mas o risco nº1 não é técnico, é o efeito-disclosure: bot revelado no início vende ~80% menos (Luo et al., Marketing Science 2019) e no BR a transparência não é opcional (LGPD/CDC).** Técnica: EL Agents tem SIP trunking nativo (TLS, G711/G722; funciona com Twilio/Telnyx), function calling, knowledge base, pt-BR via Flash v2.5 (claim 75ms é inference-only; medição independente Cekura: 1,73s p50 por turno — o melhor de 6 plataformas, mas não é "latência de ChatGPT" garantida). Custo: ~US$0,08-0,12/min de agente + telefonia; 100 ligações×4min ≈ US$40-70/mês (~R$220-380) na opção gerenciada. Vercel serverless NÃO segura o WebSocket da chamada — qualquer opção self-host (Pipecat, 13,4k stars) exige container persistente (Fly ~US$6/mês) + ~2-4 semanas de eng. Regulatório: prefixo 0303 para prospecção ativa foi tornado FACULTATIVO pela Anatel em ago/2025, mas MPF recomendou restabelecer (jan/2026) — instável; gravação exige aviso; voz é biométrico na leitura corrente da LGPD. Proposta: NÃO construir cold call por IA agora; se testar voz, testar em chamada de follow-up já esperada (lead que respondeu no WhatsApp) com disclosure honesto no abre — é onde a evidência diz que o custo do disclosure é menor. Detalhe completo no relatório da sessão de 04/ago. — elevenlabs.io/docs (SIP/models); cekura.ai; softcery.com/lab (comparativo 12 plataformas); gov.br/anatel + agenciabrasil (0303); Luo, Tong, Fang & Qu 2019, Mktg Sci 38(6)
