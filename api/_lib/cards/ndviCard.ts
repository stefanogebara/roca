 ...
 * Turns "NDVI ~0.62" into something a farmer reads in one glance.
 */
...
export function ndviSvg(data: NdviCardData): string {
  ...
  // Ramp legend bar with a marker at the reading.
  const barX = 48;
  const barY = 300;
  const barW = W - 96;
  ...
  <text x="48" y="88" font-family="DM Sans" font-size="26" font-weight="700" fill="${C.muted}">Stevi · Vigor da lavoura (satélite)</text>
  <text x="48" y="122" font-family="DM Sans" font-size="20" fill="${C.muted}">Sentinel-2 · ${esc(br(data.date))} · ${esc(scope)}</text>
